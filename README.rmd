...
Vamos ler os dados diretamente do github com auxílio da rápida função `fread` do pacote `data.table`
...
```{r read-sf, message=F, warning = F}
sf_states <- read_state()
```

Agora, vamos juntar os dois datasets utilizando a sigla de cada estado como chave primária. Dessa forma, agora todas as observações, além do dia e número de casos, contém também as respectivas geometrias.
...
## Referências
 ...
 - [“Monitoring the number of COVID-19 cases and deaths in brazil at municipal and federative units level" - W. Cota](https://10.1590/scielopreprints.362)
